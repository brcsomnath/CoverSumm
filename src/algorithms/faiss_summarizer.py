...
class FaissOnlineSummarizer(OnlineSummarizer):
  ...
  def _return_knn(self, query):
    dist, idx = self._cover_tree.search(
  ...
  def update_summary(self, input_point):
    ...
    if self._cover_tree is None:
      self._cover_tree = faiss.IndexFlatL2(len(input_point))
      self._cover_tree.add(np.array(self._points))
    else:
      self._cover_tree.add(input_point[None, :])
